...
public class InitPacketsLoadWord
{
    ...
    private void UpdateInventoryBar()
    {
        ...
        if(info != null & info.PlayerInfo.Stats != null)
        {
            EventProcessor.Instance.QueueEvent(() => { InventoryWindow.Instance.UpdateStats(info.PlayerInfo.Stats); });
        }
    ...
    private void InitSettingUserInfo()
    {
        ...
        if(userInfo.PlayerInfo.Identity.ClanId != 0)
        {
            var packet = UserPacketFactory.CreateRequestPledgeInfo(userInfo.PlayerInfo.Identity.ClanId);
