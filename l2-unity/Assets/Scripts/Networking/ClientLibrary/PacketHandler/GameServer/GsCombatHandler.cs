...
public class GsCombatHandler : ServerPacketHandler
{
    private PlayerPositionSender _pp_sender;
    private SynchronizationContext _synchronizationContext;
    public GsCombatHandler()
    {
    ...
    public override void HandlePacket(IData itemQueue)
    {
        ...
        switch (type)
        {
            case GSCombatPacketType.MyTargetSelected:
                OnMyTargetSelected(itemQueue.DecodeData());
                break;
            case GSCombatPacketType.AUTO_ATTACK_START:
                AutoAttackStart(itemQueue.DecodeData());
                break;
            case GSCombatPacketType.AUTO_ATTACK_STOP:
                AutoAttackStop(itemQueue.DecodeData());
                break;
            case GSCombatPacketType.MagicSkillUse:
                MagicSkillUse(itemQueue.DecodeData());
                break;
            case GSCombatPacketType.SetupGauge:
                SetupGauge(itemQueue.DecodeData());
                break;
            case GSCombatPacketType.MagicSkillLaunched:
                MagicSkillLaunched(itemQueue.DecodeData());
                break;
            case GSCombatPacketType.InventoryUpdate:
                OnInventoryUpdate(itemQueue.DecodeData());
                break;
            case GSCombatPacketType.EnchantResult:
                OnEnchantResult(itemQueue.DecodeData());
                break;
            case GSCombatPacketType.ItemList:
                OnCharItemList(itemQueue.DecodeData());
                break;
            case GSCombatPacketType.ChooseInventoryItem:
