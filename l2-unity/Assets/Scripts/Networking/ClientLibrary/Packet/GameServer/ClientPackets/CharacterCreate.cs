...
{
    public CharacterCreate(string name , int female , int hairStyle , int hairColor , int face , PlayerTemplates template) : base((byte)GameClientPacketType.CharacterCreate)
    {
        WriteSOther(name);
        WriteChar((char)0);
        ...
        WriteI(hairColor);
        WriteI(face);
        BuildPacketManualType((byte)GameClientPacketType.CharacterCreate);
    }
}
