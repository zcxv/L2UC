...
public class InventoryGearTab : L2Tab
{
    ...
    public override void Initialize(VisualElement chatWindowEle, VisualElement tabContainer, VisualElement tabHeader)
    {
        ...
        _gearAnchors = new Dictionary<ItemSlot, VisualElement>
        {
            ...
            { ItemSlot.boots, _windowEle.Q<VisualElement>("Boots") },
    ...
    public void UpdateItemList(List<ItemInstance> items)
    {
        ...
        items.ForEach(item =>
        {
            if (item.Equipped)
            {
                ...
                }//l2j send 2 rings bodyCode(48)
                else if(item.BodyPart == ItemSlot.lfinger)
                {
                    if (_gearSlots[item.BodyPart].IsEmpty)
                    {
                        _gearSlots[ItemSlot.lfinger].AssignItem(item);
                    }
                    else
                    {
                        _gearSlots[ItemSlot.rfinger].AssignItem(item);
                    }
                }
                else if (item.BodyPart == ItemSlot.lear)
                {
                    if (_gearSlots[item.BodyPart].IsEmpty)
                    {
                        _gearSlots[ItemSlot.lear].AssignItem(item);
                    }
                    else
                    {
                        _gearSlots[ItemSlot.rear].AssignItem(item);
                    }
                }
                else
                {
                    ...
                    {
                        if (_gearSlots.ContainsKey((ItemSlot)item.BodyPart))
                        {
